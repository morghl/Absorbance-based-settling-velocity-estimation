# -*- coding: utf-8 -*-
"""
Created on Fri Mar 31 09:29:56 2023
...
def response(t_data, v, D_over_v, model, ivp_pars, c_max, measured_span = [.005]):
...
for i, file_name in enumerate(sheet_names):
    ...
    popt, pcov = curve_fit(lambda t_data, v, D: response(t_data, v, D, model, ivp_pars, c_max, measured_span = measured_span),
                            t_data,
                            c_data,
                            p0 = [v_guess, D_over_v_guess],
                            # bounds = (1e-10, np.inf),
                            bounds = ((0, 1e-3), np.inf),
                            )
    ...
    ax_flat[i].plot(t_data, c_sim, label = f'Model, v = {popt[0]:.3} [m/s], D = {popt[1]*popt[0]:.3} [m^2/s]', color = 'k')
