...
def fem_model(t, y, C_inv, K_conv, K_disp, Z, c_max):
    ...
    dcdt = C_inv @ - (K_disp @ y - (1 - beta*(y/c_max)**Z)*K_conv @ y )
...
def response(t_data, v, D, Z, c_max, model, ivp_pars, measured_span = [.005]):
    
    sol = solve_ivp(lambda t, y: fem_model(t, y, model['C_inv'], v*model['K_conv'], D*model['K_disp'], Z, c_max), 
                    ivp_pars['t_span'], 
                    ivp_pars['c_init'], 
                    t_eval = t_data,
                    method = 'BDF',
...
for i, file_name in enumerate(sheet_names):
    ...
    popt, pcov = curve_fit(lambda t_data, v, D, Z: response(t_data, v, D, Z, c_max, model, ivp_pars, measured_span = measured_span),
                            t_data,
                            c_data,
                            p0 = [v_guess, D_guess, Z_guess],
                            # bounds = (1e-10, np.inf),
                            bounds = ((0, 0, 1e-5),(np.inf, np.inf, 5)),
                            )
    
    pars[i, :] = popt
    
    c_sim = response(t_data, popt[0], popt[1], popt[2], c_max, model, ivp_pars, measured_span = measured_span)
    ...
    ax_flat[i].plot(t_data, c_sim, label = f'Model, v = {popt[0]:.3} [m/s], D = {popt[1]:.3} [m^2/s], Z = {popt[2]:.3}', color = 'k')
...
v, D, Z = pars[zone-1, :]
# v_guess, D_guess = [8.78563940e-05, 4.83611245e-08]
# v_guess, D_guess = [4.28740177e-04, 2.88761566e-08]
c_max = exp_data['Zon' + str(zone)]['c_max']
# c_max = 5
# c_max = 116.54999999999998
